...
**IMPORTANT**: To use an Encoder, you must have a subdomain which is provided by {{COMPANY_NAME}}.
...
useEffect(() => {
  if (vc == null) {
    const opts: types.VideoClientOptions = {
      backendEndpoints: ["{{YOUR_ENDPOINT}}"], // If you do not have a {{COMPANY_NAME}} subdomain, contact {{COMPANY_NAME}}.
      ...
      {encoderUi != null && (
        <MediaContainer>
          ...
          <ControlBar>
            ...
            <JoinBroadcastButton
              setCallId={(id) => {}}
              broadcastOptions={{ streamName: "{{STREAM_NAME}}" }}
            />
            <ScreenCaptureButton />
            <FullscreenButton />
            <SettingsButton />
