...
call.pvcCall.on('webrtc-stats', (data) => {
  if (data.videoOutboundBitrate <= {{BITRATE_THRESHOLD}}) {
    // Do something useful
  }
}));
```
...
```ts
import { LoggerGlobal } from "{{LOGGER_PACKAGE}}";

const yourLogger = new LoggerGlobal();
yourLogger.setOptions({
  host: {{HOST_VARIABLE}},
  interval: {{INTERVAL_MS}}, // This refers to the interval in which your logs will be sent, 5000 === 5000 Milliseconds.
  level: "{{LOG_LEVEL}}", // Please refer to the list below for the available levels.
});

// Here you can also add some Global Aggregate and Meta
yourLogger.setGlobalAggregate("{{AGGREGATE_KEY}}", "{{AGGREGATE_VALUE}}");
yourLogger.setGlobalMeta("{{META_KEY}}", "{{META_VALUE}}");
```
...
logger.setLoggerAggregate("{{MESSAGE_KEY}}", "{{MESSAGE_VALUE}}");
...
logger.setMessageAggregate("{{MESSAGE_KEY}}", "{{MESSAGE_VALUE}}");
